...
if os.environ['machine'] == 'cloudbreak':
    sim_dir_base = '/data/cb1/Projects/P79d_ML/simulations/brano_suite'
    product_dir_base = '/data/cb1/Projects/P79d_ML/simulations/brano_suite/products'
else:
    sim_dir_base = dl.sim_dir_base
    product_dir_base = dl.product_dir_base
...
for sim in full_list:
    ...
    framelist = list(range(102))
    if sim.startswith('d15'):
        framelist = list(range(10,45))
    simulation.sim(sim, data_location="%s/%s"%(sim_dir_base,name),
                   product_location="%s/%s"%(product_dir_base,name),
                   ms=ms,ma=ma,all_frames=list(range(10,102)),framelist=list(range(10,102)))

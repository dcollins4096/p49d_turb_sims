...
from dtools import davetools as dt
...
def align(A1,A2,name_base='shot', xrange=[290,350], thresh=250, x_override=None):
    ...
    output='plots_to_sort/%s_align1'%(name_base)
    fig.savefig(output)
    print(output)
    plt.close('fig')
    ...
    fig.savefig('plots_to_sort/%s_aligned_colortest'%name_base)

    ext = dt.extents()
    ext(T1)
    ext(T2)
    norm = mpl.colors.Normalize(ext.minmax[0],ext.minmax[1])
    name1 = 'plots_to_sort/%s_shot1_trim_align.png'%name_base
    name2 = 'plots_to_sort/%s_shot2_trim_align.png'%name_base

...
pack = queb3.simulation_package( directory=sim_dir,frames=frames,prefix="as21_rmc",
                                dataset_name='as21_r_DD',frbname="./",plotdir=plot_dir)
...
for frame in frames:
    ...
    theta_phi =  [[0,31,'wrong']]
    theta_phi =  [[posang,0,'wront'] for posang in range(0,370,10)]
    #theta_phi = [90,180]
    for theta,phi,lab in theta_phi:
        ...
        ax.hist(this_proj['E'].flatten()-this_proj['E'].mean(),histtype='step',bins=100,color=angles(theta))
    #ax.legend(loc=0)

...
if 0:
    #get the symmetry
    x = np.arange(0,1,1/N)
    y = np.sin(3*2*np.pi*x)
    yhat = np.fft.fft(y)
    k = np.fft.fftfreq(x.size)
    fig,axes=plt.subplots(1,2)
    ax0=axes[0];ax1=axes[1]
    ax0.plot(x,y)
    ax1.plot(k, np.abs(yhat)    )
    fig.tight_layout()
    fig.savefig('plots_to_sort/t1')
if 0:
    #use the symmetry
    x = np.arange(0,1,1/N)
    yhat = np.zeros(N)
    yhat[8] = 1
    yhat[yhat.size//2:]=yhat[yhat.size//2:0:-1]
    y = np.fft.ifft(yhat)
    k = np.fft.fftfreq(x.size)
    fig,axes=plt.subplots(1,2)
    ax0=axes[0];ax1=axes[1]
...
if 0:
    ...
    Ahat = (np.abs(kx-Vx)<1e-5)*(np.abs(ky-Vy)<1e-5)*np.exp(1j*np.pi/2)
    #THIS WORKS
    H = Ahat.shape[0]//2
    Ahat[H:,H:]=np.conj(Ahat[H:0:-1,H:0:-1])
    Ahat[H:,1:H+1]=np.conj(Ahat[H:0:-1,:H-1:-1])
    #############
    print('maybe')
    print( np.where( np.abs(Ahat) > 1e-5))
    A = np.fft.ifftn(Ahat)

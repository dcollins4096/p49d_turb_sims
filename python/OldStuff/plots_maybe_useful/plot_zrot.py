...
class slopes():
    ...
    def fit(self,lcent,fitrange):
        for field in ['avg_cltt','avg_clee','avg_clbb']:
...
if 'spectra_dict' not in dir() or clobber:
    shortprefix='time'
    simlist=['rmc_as21_zrot']
    for i, simdes in enumerate(simlist):
        spectra_dict={}
        theta = 90
        frames=[1]
        for phi in range(0,-100,-10):
            spectra_dict[phi]=slopes(simdes) 
            ...
            proj=pack.read_queb(frame=frames[0],theta=theta,phi=phi,bin_style='dx1')
            fitrange=proj.determine_fit_range()  #something better

            spectra_fname = "avg_spectra_%s_%s.h5"%(simdes,phi)
            print(spectra_fname)
            if os.path.exists(spectra_fname):
                try:
                    fptr=h5py.File(spectra_fname,'r')
                    for field in fptr:
                        spectra_dict[phi].spectra[field]=fptr[field][()]
                except:
                    raise
                finally:
                    fptr.close()
            spectra_dict[phi].fit(proj.lcent,fitrange)
#
# Spectra plots
#
...
if 1:
    ...
    axlist=ax.flatten()
    philist = range(0,-100,-10)
    rm = dt.rainbow_map(len(philist))

    for nf,field in enumerate(['avg_cltt','avg_clee','avg_clbb']):
        for nphi,phi in enumerate(philist):
            axlist[nf].plot(proj.lcent, spectra_dict[phi].spectra[field], c=rm(nphi))
    for a in axlist:
    ...
    fig.savefig('%s/PhiTest.pdf'%plotdir)

...
def make_spectra_files():
    for phi in range(0,-100,-10):
        ...
        for i,sim in enumerate(simlist):
            ...
            avg_reb=0
            projections=[]
            longprefix='%s_%s_%s'%(simdes,shortprefix,phi)
            frames=[1]
            theta=90
            ...
            fitrange=None;#proj.determine_fit_range()  #something better
            if not os.path.exists(spectra_fname) or True:
                ...
                #print(frame,simdes)
                    ...
                    proj=pack.read_queb(frame=frame,theta=theta,phi=phi,bin_style='dx1')
                    if fitrange is None:
                        fitrange=proj.determine_fit_range()
                    if proj is None:
                        continue
                    nplots+=1

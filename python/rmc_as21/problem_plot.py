...
for fname in glob.glob('*image_r*.out'):
    print("Reading",fname)

    a = readImage(fname)
    theta = int(fname.split(".")[0].split("_")[2])
    phi   = int(fname.split(".")[0].split("_")[3])
    print(theta,phi)
    for stokes in ['Q','U','I']:
        ...
        oname =  "as21_r_DD%04d_th%04d_ph%04d_%s.png"% (1,theta,phi,stokes)
        onamef = "as21_r_DD%04d_th%04d_ph%04d_%s.fits"%(1,theta,phi,stokes)
        plt.savefig(oname)

    ...
    for nf,field in enumerate(field_list):
        ...
        for ns,sim in enumerate(simlist):
            ...
            for LOS in LOS_LIST:
                ...
                hist_3, bins_3, visthings_3=ax_right.hist( slope_list_3, histtype='step', orientation='horizontal',bins=bins)
                hist_1, bins_1, visthings_1=ax_right.hist( slope_list, histtype='step', orientation='horizontal',bins=bins)
                ...
                if 1:
                    most_probable_bin = np.argmax(hist_3)
                    bL = bins_3[most_probable_bin]
                    bR = bins_3[most_probable_bin+1]
                    SL = nar(slope_list_3)
                ...
                if 1:
                    most_probable_bin = np.argmax(hist_1)
                    bL = bins_1[most_probable_bin]
                    bR = bins_1[most_probable_bin+1]
                    SL = nar(slope_list)
                    most_probable =  (SL >= bL)*(SL <= bR)
                    a_mean=a_list.mean()
                    b_mean=a_list.mean()
                    ...
                    AlphaC_1 = SL[most_probable].mean()
                ...
                #
                # Most probable linear
                #
                this_l=10**refit3.line(np.log10(xvals), popt_1[0],AlphaC_1)
                kfit = lcentf[16]
                index = np.argmin( np.abs( xvals-kfit))
                this_l = this_l*specf[16]/this_l[index]
                chi2 = np.sum( (this_l-specf[ok])**2/specf[ok])
